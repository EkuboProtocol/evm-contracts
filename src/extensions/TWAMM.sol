                    ...
                    if (amount0 != 0 && amount1 != 0) {
                        ...
                        if (sqrtRatioNext > corePoolState.sqrtRatio()) {
                            ...
                            (swapDelta0, swapDelta1, corePoolState) =
                                CoreLib.swap(CORE, poolKey, int128(uint128(amount1)), true, sqrtRatioNext, 0);
                        } else if (sqrtRatioNext < corePoolState.sqrtRatio()) {
                            (swapDelta0, swapDelta1, corePoolState) =
                                CoreLib.swap(CORE, poolKey, int128(uint128(amount0)), false, sqrtRatioNext, 0);
                        }
                        ...
                        if (amount0 != 0) {
                            (rewardDelta0, rewardDelta1, corePoolState) =
                                CoreLib.swap(CORE, poolKey, int128(uint128(amount0)), false, MIN_SQRT_RATIO, 0);
                        } else {
                            (rewardDelta0, rewardDelta1, corePoolState) =
                                CoreLib.swap(CORE, poolKey, int128(uint128(amount1)), true, MAX_SQRT_RATIO, 0);
                        }
