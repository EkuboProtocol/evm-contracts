...
interface IBoostedFees is IExposedStorage, IExtension, ILocker, IForwardee {
    ...
    /// @param startTime Incentive start time (inclusive)
    /// @param endTime Incentive end time (exclusive)
    /// @param rate0 Per-second incentive rate for token0 as a fixed point *.32 value
    /// @param rate1 Per-second incentive rate for token1 as a fixed point *.32 value
    event PoolBoosted(PoolId indexed poolId, uint64 startTime, uint64 endTime, uint256 rate0, uint256 rate1);
    ...
    /// @notice Reverts when the computed rate is zero for both tokens
