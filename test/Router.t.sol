...
contract RouterTest is FullTest {
    function test_basicSwap_token0_in_reverts_slippage(CallPoints memory callPoints) public {
        PoolKey memory poolKey = createPool(0, 1 << 127, 100, callPoints);
        createPosition(poolKey, Bounds(-100, 100), 1000, 1000);

        vm.expectRevert(abi.encodeWithSelector(Router.MinimumOutputNotReceived.selector, 0, 49, 50));
        router.swap(
    ...
    function test_basicSwap_token0_out_reverts_slippage(CallPoints memory callPoints) public {
        PoolKey memory poolKey = createPool(0, 1 << 127, 100, callPoints);
        createPosition(poolKey, Bounds(-100, 100), 1000, 1000);

        vm.expectRevert(abi.encodeWithSelector(Router.MaximumInputExceeded.selector, 0, 202, 100));
        router.swap(
            RouteNode({poolKey: poolKey, sqrtRatioLimit: 0, skipAhead: 0}),
            TokenAmount({token: address(token0), amount: -100}),
            100
    ...
    function test_basicSwap_token1_in_reverts_slippage(CallPoints memory callPoints) public {
        PoolKey memory poolKey = createPool(0, 1 << 127, 100, callPoints);
        createPosition(poolKey, Bounds(-100, 100), 1000, 1000);
    ...
    function test_basicSwap_token1_out_reverts_slippage(CallPoints memory callPoints) public {
        PoolKey memory poolKey = createPool(0, 1 << 127, 100, callPoints);
        createPosition(poolKey, Bounds(-100, 100), 1000, 1000);
    ...
    function test_multihopSwap(CallPoints memory callPoints) public {
        ...
        Delta[] memory d = router.multihopSwap(Swap(route, TokenAmount({token: address(token0), amount: 100}), 0));
    ...
    function test_multiMultihopSwap(CallPoints memory callPoints) public {
        ...
        swaps[0] = Swap(route0, TokenAmount({token: address(token0), amount: 100}), 0);
        swaps[1] = Swap(route1, TokenAmount({token: address(token0), amount: -100}), 0);
