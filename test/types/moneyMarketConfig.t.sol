...
contract MoneyMarketConfigTest is Test {
    ...
    function test_parse(uint32 _ltvX32, uint32 _twapDuration, uint32 _liquidationDuration, uint8 _minLiquidityMagnitude)
        public
        pure
    {
        MoneyMarketConfig config = createMoneyMarketConfig({
        ...
        (uint32 ltv, uint32 twap, uint32 liquidation, uint8 minLiquidityMag) = config.parse();
